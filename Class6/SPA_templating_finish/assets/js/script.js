...
var checkboxes = $('.all-students input[type = checkbox]');
	...
	function attachEvents() {
      ...
      //if it is not checked
			if(!that.is(":checked")){
				 if(filters[cat]&& filters[cat].length && (filter[cat].index(that.val()) != -1)){
	...
	//Get the filters object turn it into a string and write it into the hashchange
...
//change the URL
	...
	//This is going to filter the components everytime you select a checkbox, depending on the filter
				...
				filters[c].forEach(function(filter){
					...
					student.forEach(function(item){
						if(typeof item.project[c] == 'string'){
							...
							if(item.project[c].toLowerCase().index(filter)!= -1){
